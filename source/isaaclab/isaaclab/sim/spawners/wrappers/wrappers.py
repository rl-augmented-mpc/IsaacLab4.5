...
def spawn_many_asset(
    ...
    scale: list[list[tuple[float, float, float]]] | None = None,
) -> Usd.Prim:
    """Spawn multiple assets based on the provided configurations.
    ...
    if scale is None:
        scale = [[(1.0, 1.0, 1.0)]*len(translation[0])]*len(translation)
    ...
    if len(scale) != len(cfg.assets_cfg):
        raise RuntimeError(
            f"Number of scales ({len(scale)}) must match the number of assets ({len(cfg.assets_cfg)})."
        )
    ...
    for i, asset_cfg in enumerate(cfg.assets_cfg):
        ...
        if len(scale[i]) != cfg.num_assets[i]:
            raise RuntimeError(
                f"Number of scales for asset {i} ({len(scale[i])}) must match the number of instances"
                f" ({cfg.num_assets[i]})."
            )
    ...
    with Sdf.ChangeBlock():
        for index, prim_path in enumerate(prim_paths):
            # spawn single instance
            # randomly select an asset configuration
            # prim_utils.define_prim(prim_path, "Xform", True)
            for proto_index, proto_prim_path in enumerate(proto_prim_paths):
                for i in range(cfg.num_assets[proto_index]):
                    ...
                    scale_spec = env_spec.GetAttributeAtPath(f"{prim_path}_{proto_index}_{i}" + ".xformOp:scale")
                    scale_spec.default = Gf.Vec3f(*scale[proto_index][i])
    # create prim 
    prim_utils.create_prim(prim_path, "Scope")
    # delete the dataset prim after spawning
    prim_utils.delete_prim(template_prim_path)

...
class StaticColliderObject(AssetBase):
    ...
    def write_mass_to_sim(self, mass: torch.Tensor, env_ids: Sequence[int] | None = None):
        """Set the mass over selected environment indices into the simulation.

        Args:
            mass: Mass values. Shape is (len(env_ids),).
            env_ids: Environment indices. If None, then all indices are used.
        """
        if env_ids is None:
            env_ids = self._ALL_INDICES
        self.root_physx_view.set_masses(mass, indices=env_ids)
    ...
    def _initialize_impl(self):
        ...
        print("template_prim", template_prim)  
        ...
        print(f"[INFO] Rigid body initialized at: {self.cfg.prim_path} with root '{root_prim_path_expr}'.")
        print(f"[INFO] Number of instances: {self.num_instances}")

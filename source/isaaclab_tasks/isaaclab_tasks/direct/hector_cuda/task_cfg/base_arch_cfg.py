...
class BaseArchCfg(DirectRLEnvCfg):
    ...
    state_space = 44 # critic observation space
    action_space = 3 # action space
    num_joint_actions:int = 10
    num_states:int = 43
    
    # TODO: override this in the derived class
    # observation, action settings
    action_lb:list[float] = [-1.0, -1.0, -1.0]
    action_ub:list[float] = [1.0, 1.0, 1.0]
    observation_lb:float = -50.0
    observation_ub:float = 50.0
    ...
    # robot spawner
    center = (terrain.center_position[0], terrain.center_position[1], 0.2)
    max_x = 2.0
    max_y = 2.0

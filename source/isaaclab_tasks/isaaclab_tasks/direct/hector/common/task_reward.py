...
def compute_exponential_reward(
    ...
    return scale*torch.exp(-torch.square(error)/std)
...
def compute_gaussian_reward(
    ...
    return scale*torch.exp(-torch.square(error)/(std**2))
...
class AliveReward:
    ...
    def compute_reward(self, reset_terminated:torch.Tensor, current_step:torch.Tensor, max_step:int)->torch.Tensor:
        return self.alive_weight * (1 - reset_terminated.float())
...
class SagittalFPSimilarityReward:
    ...
    def compute_reward(self, residual_sagittal_fp:torch.Tensor):
        ...
        if self.fp_reward_mode == "linear":
            ...
            fp_reward = compute_exponential_reward(fp_error, scale=1.0, std=self.fp_similarity_coeff)
        elif self.fp_reward_mode == "gaussian":
            fp_reward = compute_gaussian_reward(fp_error, scale=1.0, std=self.fp_similarity_coeff)
...
class VelocityTrackingReward:
    ...
    def compute_reward(self, 
        ...
        if self.height_reward_mode == "linear":
            ...
            height_reward = torch.sum(compute_exponential_reward(height_error, scale=1.0, std=self.height_similarity_coeff), -1)
        elif self.height_reward_mode == "gaussian":
            height_reward = torch.sum(compute_gaussian_reward(height_error, scale=1.0, std=self.height_similarity_coeff), -1)
        ...
        if self.lin_vel_reward_mode == "linear":
            ...
            lin_vel_reward = torch.sum(compute_exponential_reward(lin_vel_error, scale=1.0, std=self.lin_vel_similarity_coeff), -1)
        elif self.lin_vel_reward_mode == "gaussian":
            lin_vel_reward = torch.sum(compute_gaussian_reward(lin_vel_error, scale=1.0, std=self.lin_vel_similarity_coeff), -1)
        ...
        if self.ang_vel_reward_mode == "linear":
            ...
            ang_vel_reward = torch.sum(compute_exponential_reward(ang_vel_error, scale=1.0, std=self.ang_vel_similarity_coeff), -1)
        elif self.ang_vel_reward_mode == "gaussian":
            ang_vel_reward = torch.sum(compute_gaussian_reward(ang_vel_error, scale=1.0, std=self.ang_vel_similarity_coeff), -1)
...
class PoseTrackingReward:
    ...
    def compute_reward(self, 
        ...
        if self.position_reward_mode == "linear":
            ...
            position_reward = torch.sum(compute_exponential_reward(position_error, scale=1.0, std=self.position_coeff), -1)
        elif self.position_reward_mode == "gaussian":
            position_reward = torch.sum(compute_gaussian_reward(position_error, scale=1.0, std=self.position_coeff), -1)
        ...
        if self.yaw_reward_mode == "linear":
            ...
            yaw_reward = torch.sum(compute_exponential_reward(yaw_error, scale=1.0, std=self.yaw_coeff), -1)
        elif self.yaw_reward_mode == "gaussian":
            yaw_reward = torch.sum(compute_gaussian_reward(yaw_error, scale=1.0, std=self.yaw_coeff), -1)
...
class SwingFootTrackingReward:
    ...
    def compute_reward(self, 
        ...
        if self.swing_foot_reward_mode == "linear":
            ...
            swing_foot_reward = torch.sum(compute_exponential_reward(swing_foot_error, scale=1.0, std=self.swing_foot_coeff), 1)
        elif self.swing_foot_reward_mode == "gaussian":
            swing_foot_reward = torch.sum(compute_gaussian_reward(swing_foot_error, scale=1.0, std=self.swing_foot_coeff), 1)
...
class GoalTrackingReward:
    ...
    def compute_reward(self, 
        ...
        if self.height_reward_mode == "linear":
            ...
            height_reward = torch.sum(compute_exponential_reward(height_error, scale=1.0, std=0.5), -1)
        elif self.height_reward_mode == "gaussian":
            height_reward = torch.sum(compute_gaussian_reward(height_error, scale=1.0, std=0.5), -1)
        ...
        if self.goal_pos_dist_reward_mode == "linear":
            ...
            goal_pos_dist_reward = torch.sum(compute_exponential_reward(goal_pos_error, scale=1.0, std=0.05), -1)
        elif self.goal_pos_dist_reward_mode == "gaussian":
            goal_pos_dist_reward = torch.sum(compute_gaussian_reward(goal_pos_error, scale=1.0, std=0.05), -1)
        ...
        if self.goal_yaw_dist_reward_mode == "linear":
            ...
            goal_yaw_dist_reward = torch.sum(compute_exponential_reward(goal_yaw_error, scale=1.0, std=0.05), -1)
        elif self.goal_yaw_dist_reward_mode == "gaussian":
            goal_yaw_dist_reward = torch.sum(compute_gaussian_reward(goal_yaw_error, scale=1.0, std=0.05), -1)
...
class ContactTrackingReward:
    ...
    def compute_reward(self, 
        ...
        if self.contact_reward_mode == "linear":
            ...
            contact_reward = compute_exponential_reward(contact_error, scale=1.0, std=self.contact_similarity_coeff)
        elif self.contact_reward_mode == "gaussian":
            contact_reward = compute_gaussian_reward(contact_error, scale=1.0, std=self.contact_similarity_coeff)
...
if __name__ == "__main__":
    ...
    gaussian_reward = compute_gaussian_reward(error, std=2.0, scale=1.0)
    exp_reward = compute_exponential_reward(error, std=2.0, scale=1.0)

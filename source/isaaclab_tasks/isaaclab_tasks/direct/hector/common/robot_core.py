...
class RobotCore:
    def __init__(self, articulation:Articulation, num_envs:int=1, foot_patch_num:int=8)->None:
    ...
    def foot_pos_local(self)->torch.Tensor:
        ...
        rot_mat = self._init_rot.clone().unsqueeze(1).repeat(1, self.foot_patch_num, 1, 1).view(-1, 3, 3)
        return torch.bmm(rot_mat.transpose(1,2), foot_pos.view(-1, 3, 1)).view(-1, self.foot_patch_num, 3)
    ...
    def foot_pos_b(self)->torch.Tensor:
        ...
        rot_mat = self.root_rot_mat_w.clone().unsqueeze(1).repeat(1, self.foot_patch_num, 1, 1).view(-1, 3, 3)
        return torch.bmm(rot_mat.transpose(1,2), foot_pos.view(-1, 3, 1)).view(-1, self.foot_patch_num, 3)
    ...
    def foot_vel_local(self)->torch.Tensor:
        ...
        rot_mat = self._init_rot.clone().unsqueeze(1).repeat(1, self.foot_patch_num, 1, 1).view(-1, 3, 3)
        foot_vel = torch.bmm(rot_mat.transpose(1, 2), foot_vel.view(-1, 3, 1)).view(-1, self.foot_patch_num, 3)
        return foot_vel

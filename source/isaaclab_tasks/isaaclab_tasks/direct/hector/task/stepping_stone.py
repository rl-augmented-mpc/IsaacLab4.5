...
class SteppingStone(HierarchicalArch):
    ...
    def log_action(self)->None:
        ...
        if self.common_step_counter % self.cfg.num_steps_per_env:
            stepping_frequency = self._actions_op[0, 0:10].cpu().numpy()
            foot_height = self._actions_op[0, 10:20].cpu().numpy()
    ...
    def _reset_robot(self, env_ids: Sequence[int])->None:
        # get center position of each sub-terrains
        ...
        self.history_buffer.reset(env_id=env_ids) # type: ignore